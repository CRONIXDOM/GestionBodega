...
public class MateriaPrimaServiceImpl implements IMateriaPrimaService {
	...
	public List<MateriaPrimaResponseDto> listarMateriaPrima() {
		return webCliente.get().uri("/materiaPrima").retrieve().bodyToFlux(MateriaPrimaResponseDto.class).collectList()
				.block();
	}
	...
	public MateriaPrimaResponseDto buscarMateriaPrimaId(Integer id) {
		return webCliente.get().uri(ub -> ub.path("/materiaPrima/buscarId/{id}").build(id)).retrieve()
				.bodyToMono(MateriaPrimaResponseDto.class).block();
	}

	@Override
	public void eliminarMateriaPrima(Integer id) {
		webCliente.delete().uri(ub -> ub.path("/materiaPrima/{id}").build(id)).retrieve().toBodilessEntity().block();
	}
