...
public class ControlCalidadServiceImpl implements IControlCalidadService {
    ...
    public List<ControlCalidadResponseDto> listarControlCalidad() {
        return webCliente.get().uri("/controlCalidad").retrieve()
                .bodyToFlux(ControlCalidadResponseDto.class).collectList().block();
    }
    ...
    public ControlCalidadResponseDto buscarControlCalidadId(Integer id) {
        return webCliente.get().uri(ub -> ub.path("/controlCalidad/buscarId/{id}").build(id))
                .retrieve().bodyToMono(ControlCalidadResponseDto.class).block();
    }

    @Override
    public void eliminarControlCalidad(Integer id) {
        webCliente.delete().uri(ub -> ub.path("/controlCalidad/{id}").build(id))
    ...
    public List<OpcionSelectDto> listarOpciones() {
        return listarControlCalidad().stream()
                .map(op -> new OpcionSelectDto(op.getIdControl(), "Control " + op.getIdControl()))
                .toList();
    }
}
