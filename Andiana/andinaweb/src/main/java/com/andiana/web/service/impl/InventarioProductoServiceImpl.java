...
public class InventarioProductoServiceImpl implements IInventarioProductoService {
	...
	public List<InventarioProductoResponseDto> listarInventario() {
		return webCliente.get().uri("/inventario").retrieve().bodyToFlux(InventarioProductoResponseDto.class)
				.collectList().block();
	}
	...
	public InventarioProductoResponseDto buscarInventarioId(Integer id) {
		return webCliente.get().uri(ub -> ub.path("/inventario/buscarId/{id}").build(id)).retrieve()
				.bodyToMono(InventarioProductoResponseDto.class).block();
	}

	@Override
	public void eliminarInventario(Integer id) {
		webCliente.delete().uri(ub -> ub.path("/inventario/{id}").build(id)).retrieve().toBodilessEntity().block();
	}

	@Override
	public List<OpcionSelectDto> listarOpciones() {
		return listarInventario().stream()
				.map(op -> new OpcionSelectDto(op.getIdInventario(), "Registro " + op.getIdInventario())).toList();
	}
}
