...
public class OrdenProduccionServiceImpl implements IOrdenProduccionService {
	...
	public List<OrdenProduccionResponseDto> listarOrden() {
		return webCliente.get().uri("/orden").retrieve().bodyToFlux(OrdenProduccionResponseDto.class).collectList()
				.block();
	}
	...
	public OrdenProduccionResponseDto buscarOrdenId(Integer id) {
		return webCliente.get().uri(ub -> ub.path("/orden/buscarId/{id}").build(id)).retrieve()
				.bodyToMono(OrdenProduccionResponseDto.class).block();
	}

	@Override
	public void eliminarOrden(Integer id) {
		webCliente.delete().uri(ub -> ub.path("/orden/{id}").build(id)).retrieve().toBodilessEntity().block();
	}
	...
	public List<OpcionSelectDto> listarOpciones() {
		...
		return listarOrden().stream().map(op -> new OpcionSelectDto(op.getIdOrden(), "Orden " + op.getIdOrden() + " · "
				+ nombreDelProducto(productos, op.getIdProducto()) + " · " + op.getFechaProgramada())).toList();
	}
