...
public class ConsultaServiceImpl implements IConsultaService {
	...
	public List<MateriaEnRecetaDto> materiasDeLaReceta(Integer idReceta) {
		return webCliente.get().uri(ub -> ub.path("/consulta/receta/{id}/materias").build(idReceta)).retrieve()
				.bodyToFlux(MateriaEnRecetaDto.class).collectList().block();
	}

	@Override
	public List<ConteoRecetaDto> conteoDeMateriasPorReceta() {
		return webCliente.get().uri("/consulta/recetas/conteo-materias").retrieve().bodyToFlux(ConteoRecetaDto.class)
				.collectList().block();
	}
