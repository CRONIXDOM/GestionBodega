...
public class LoteProduccionServiceImpl implements ILoteProduccionService {
    ...
    public List<LoteProduccionResponseDto> listarLote() {
        return webCliente.get().uri("/lote").retrieve()
                .bodyToFlux(LoteProduccionResponseDto.class).collectList().block();
    }
    ...
    public LoteProduccionResponseDto buscarLoteId(Integer id) {
        return webCliente.get().uri(ub -> ub.path("/lote/buscarId/{id}").build(id))
                .retrieve().bodyToMono(LoteProduccionResponseDto.class).block();
    }
    ...
    public List<OpcionSelectDto> listarOpciones() {
        return listarLote().stream()
                .map(op -> new OpcionSelectDto(op.getIdLote(), op.getNumeroLote()))
                .toList();
    }
}
