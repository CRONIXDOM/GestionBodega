...
public class RecetaProduccionServiceImpl implements IRecetaProduccionService {
    ...
    public List<RecetaProduccionResponseDto> listarReceta() {
        return webCliente.get().uri("/receta").retrieve()
                .bodyToFlux(RecetaProduccionResponseDto.class).collectList().block();
    }
    ...
    public RecetaProduccionResponseDto buscarRecetaId(Integer id) {
        return webCliente.get().uri(ub -> ub.path("/receta/buscarId/{id}").build(id))
                .retrieve().bodyToMono(RecetaProduccionResponseDto.class).block();
    }

    @Override
    public void eliminarReceta(Integer id) {
        webCliente.delete().uri(ub -> ub.path("/receta/{id}").build(id))
                .retrieve().toBodilessEntity().block();
    }

    /** Una receta se reconoce por su producto y su versión, no por su número. */
    @Override
    public List<OpcionSelectDto> listarOpciones() {
        List<ProductoResponseDto> productos = servicioProducto.listarProducto();

        return listarReceta().stream()
                .map(op -> new OpcionSelectDto(op.getIdReceta(),
                        nombreDelProducto(productos, op.getIdProducto()) + " · versión " + op.getVersion()))
                .toList();
    }
