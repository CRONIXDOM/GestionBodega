...
public class MovimientoMateriaPrimaServiceImpl implements IMovimientoMateriaPrimaService {
    ...
    public List<MovimientoMateriaPrimaResponseDto> listarMovimiento() {
        return webCliente.get().uri("/movimiento").retrieve()
                .bodyToFlux(MovimientoMateriaPrimaResponseDto.class).collectList().block();
    }
    ...
    public MovimientoMateriaPrimaResponseDto buscarMovimientoId(Integer id) {
        return webCliente.get().uri(ub -> ub.path("/movimiento/buscarId/{id}").build(id))
                .retrieve().bodyToMono(MovimientoMateriaPrimaResponseDto.class).block();
    }

    @Override
    public void eliminarMovimiento(Integer id) {
        webCliente.delete().uri(ub -> ub.path("/movimiento/{id}").build(id))
    ...
    public List<OpcionSelectDto> listarOpciones() {
        return listarMovimiento().stream()
                .map(op -> new OpcionSelectDto(op.getIdMovimiento(), "Movimiento " + op.getIdMovimiento()))
                .toList();
    }
}
